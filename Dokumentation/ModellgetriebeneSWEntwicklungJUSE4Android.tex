...
\abstract{ Die Geschwindigkeit bei der Entwicklung von Applikationen, besonders für Mobile Systeme ist in den letzten Jahre rasant gestiegen. Dadurch rücken Entwicklungstechniken, wie Model-Driven Architecture(MDA) und Model-Driven Engineering (MDE) wieder weiter in den Fokus der Entwickler und der Industrie. Sie versprechen Entwicklung einer Anwendung durch die Modellierung mit UML-Diagrammen. In dieser Arbeit wurde die Applikation Juse4Android herangezogen, welches in der Lage ist aus der Spezifizierung eines Klassendiagrammes eine Mobile Anwendung für Android zu generieren.  Dafür wird ein UML-Klassen Diagramm in der Beschreibungssprache USE (UML Specification Environment) entwickelt und dieses mittels \textit{Juse4Android} in eine Mobile Android Anwendung übersetzt. Die USE-Beschreibung wird durch die Object Constraint Language(OCL) erweitert, um das Klassendiagramm weiter zu präzisieren. Es folgt die Untersuchung der generierten Applikation nach Architektur und Erweiterbarkeit. Im Zuge dessen wird neue Funktionalität manuell in die App integriert. Zum Schuss wird ein Fazit über Einsetzbarkeit und zukünftige Verwendung gezogen.
%modellgetriebene Entwicklung vielversprechend;  Entwicklung einer Applikation, USE-Spezifikation zur Definition eines Klassendiagramms; OCL-Constraints oben drauf;Im Anschluss Generierung der Anwendung mit Hilfe von JUSE4Android. Untersuchung der Anwendung anhand bestimmter Fragestellungen
}
