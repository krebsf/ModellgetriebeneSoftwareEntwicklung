...
\abstract{ Die Geschwindigkeit bei der Entwicklung von Applikationen, besonders für Mobile Systeme, ist in den letzten Jahren rasant gestiegen. Dadurch rücken Entwicklungstechniken wie Model-Driven Architecture(MDA) und Model-Driven Engineering (MDE) wieder weiter in den Fokus der Entwickler und der Industrie. Sie versprechen Entwicklung einer Anwendung durch die Modellierung mit UML-Diagrammen. In dieser Arbeit wird die Applikation \textit{JUSE4Android} herangezogen, welches in der Lage ist aus der Spezifizierung eines Klassendiagrammes eine Mobile Anwendung für Android zu generieren.  Dafür wird ein UML-Klassen Diagramm in der Beschreibungssprache USE (UML Specification Environment) entwickelt und dieses mittels \textit{JUSE4Android} in eine Android-Anwendung übersetzt. Die USE-Beschreibung wird durch die Object Constraint Language (OCL) erweitert, um das Klassendiagramm zu präzisieren. Es folgt die Untersuchung der generierten Applikation nach Architektur und Erweiterbarkeit. Im Zuge dessen wird neue Funktionalität manuell in die App integriert. 
