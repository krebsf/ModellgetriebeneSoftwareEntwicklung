...
public class ProjectActivity extends MasterActivity implements
			...
			PropertyChangeListener
{
	...
	public void startActivity_ToONE(Project project)
	{
		if(!restarted)
		{
			if(ONCREATION)
				setDetailFragment(ProjectDetailFragment.ARG_VIEW_NEW, null);
			else
				setDetailFragment(ProjectDetailFragment.ARG_VIEW_DETAIL, project);
		}
	}
	...
	public <T> void startActivity_ToMANY(Collection<T> subSet, Bundle savedInstanceState)
	{
		if(!restarted)
		{
			...
			{
				list_fragment.setListAdapter(new ListAdapter(this, new ProjectListViewHolder(), Project.allInstances()));
			}
		}
	}
	
	public void setDetailFragment(String View, Project project)
	{
		detail_fragment = new ProjectDetailFragment();
		if(project != null)
			UtilNavigate.replaceFragment(this, detail_fragment, R.id.project_detail_container, UtilNavigate.setFragmentBundleArguments(View,project.ID()));
		else
			UtilNavigate.replaceFragment(this, detail_fragment, R.id.project_detail_container, UtilNavigate.setFragmentBundleArguments(View,0));
	...
	public void orientationSettings(Project clickedProject)
	{
		if(!mTwoPane && AssociationEndMultiplicity != ToONE)
		{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		{
			mTwoPane = true;
			setContentView(R.layout.project_layout_twopane);
		}
		else
			setContentView(R.layout.project_layout_onepane);
		...
		if(savedInstanceState == null)
		{
			if (AssociationEndMultiplicity != ToONE)
			{
				...
				ft.add(R.id.project_list_container, list_fragment);
			}
			navigation_bar = new ProjectNavigationBarFragment();
			ft.add(R.id.project_navigationbar_container, navigation_bar);
			ft.commit();
		}
		else
		{
			if (AssociationEndMultiplicity != ToONE)
				list_fragment = (ListFragmentController) getSupportFragmentManager().findFragmentById(R.id.project_list_container);
			navigation_bar = (Fragment) getSupportFragmentManager().findFragmentById(R.id.project_navigationbar_container);
			detail_fragment = (Fragment) getSupportFragmentManager().findFragmentById(R.id.project_detail_container);
		...
		if(extras() != null)
		{
			if(extras().containsKey(projects_MemberAssociation))
			{
				...
				{
					mTwoPane = false;
					clickedProject = (Project)member.projects();
					startActivity_ToONE(clickedProject);
				}
			}
		...
		if(AssociationEndMultiplicity != ToONE)
		{
			Project.getAccess().setChangeListener(list_fragment);
		}
	}
	...
	public void onStart()
	{
		...
		if(AssociationEndMultiplicity != ToONE)
		{
			...
			{
				clickedProject = (Project) list_fragment.getListAdapter().getItem(list_fragment.getSelectedPosition());
				
			}
			
		}
		((NavigationBarFragment)navigation_bar).setViewingObject(clickedProject);
		orientationSettings(clickedProject);
	}
	...
	{
		super.onResume();
		Transactions.addSpecialCommand(new Command(ProjectActivity.class, CommandType.READ, CommandTargetLayer.VIEW, 0, null, null, null, 0, null, null));
		if(ONCREATION)
			Transactions.StartTransaction();
		restarted = false;
	}
	...
	public void onBackPressed()
	{
		...
		else
		{
			...
			Transactions.addSpecialCommand(new Command(ProjectActivity.class, CommandType.BACK, CommandTargetLayer.VIEW, 0, null, null, null, 0, null, null));
			super.onBackPressed();
		}
	}
	...
	{
		Project.getAccess().removeChangeListener(list_fragment);
		super.onDestroy();
	}
	...
	public void onItemSelected(int listPos, Object object, boolean mSinglePanelongclick)
	{
		...
		{
			this.clickedProject = (Project) object;
			((NavigationBarFragment)navigation_bar).setViewingObject(clickedProject);
			
			if (mTwoPane)
			{
				setDetailFragment(ProjectDetailFragment.ARG_VIEW_DETAIL, clickedProject);
			}
			...
			case R.id.menu_confirm_oncreation:
				if(ONCREATION)
				{
					if(list_fragment.getSelectedPosition() != ListView.INVALID_POSITION)
					{
						if(member != null)
						{
							setResult(Activity.RESULT_OK, new Intent().putExtra(projects_MemberAssociation, clickedProject.ID()));
							finish();
						}
						if(worker != null)
						{
							setResult(Activity.RESULT_OK, new Intent().putExtra(MemberAssociation, clickedProject.ID()));
							finish();
						}
						if(qualification != null)
						{
							setResult(Activity.RESULT_OK, new Intent().putExtra(RequiresAssociation, clickedProject.ID()));
							finish();
						}
						if(company != null)
						{
							setResult(Activity.RESULT_OK, new Intent().putExtra(CarriesOutAssociation, clickedProject.ID()));
							finish();
						}
					}
					else
						UtilNavigate.showWarning(this, "Wrong action", "Error - In order to confirm a Project you must have a Project selected\nSolution - In order to finish the association action\n\t1 - select a Project\n\t2 - if no Project is available create a new one by means of the plus icon");
				}
				break;
			case R.id.menu_edit:
				if (AssociationEndMultiplicity == ToONE || list_fragment.getSelectedPosition() != ListView.INVALID_POSITION)
				{
					setDetailFragment(ProjectDetailFragment.ARG_VIEW_EDIT,	clickedProject);
				}
				break;
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		...
		{
			clickedProject.insertAssociation((Member) Member.getMember((Integer) data.getExtras().get(projects_MemberAssociation)), projects_MemberAssociation);
			if(!Transactions.StopTransaction())
				Transactions.ShowErrorMessage(this);
		}
		if (requestCode == CREATION_CODE && resultCode == Activity.RESULT_OK && data.getExtras().containsKey(MemberAssociation))
		{
			clickedProject.insertAssociation((Worker) Worker.getWorker((Integer) data.getExtras().get(MemberAssociation)), MemberAssociation);
			if(!Transactions.StopTransaction())
				Transactions.ShowErrorMessage(this);
		}
		if (requestCode == CREATION_CODE && resultCode == Activity.RESULT_OK && data.getExtras().containsKey(RequiresAssociation))
		{
			clickedProject.insertAssociation((Qualification) Qualification.getQualification((Integer) data.getExtras().get(RequiresAssociation)), RequiresAssociation);
			if(!Transactions.StopTransaction())
				Transactions.ShowErrorMessage(this);
		}
		if (requestCode == CREATION_CODE && resultCode == Activity.RESULT_OK && data.getExtras().containsKey(CarriesOutAssociation))
		{
			clickedProject.insertAssociation((Company) Company.getCompany((Integer) data.getExtras().get(CarriesOutAssociation)), CarriesOutAssociation);
			if(!Transactions.StopTransaction())
				Transactions.ShowErrorMessage(this);
		}
	...
	public void onDetailOK(boolean isNew, Project newProject)
	{
		if(Transactions.StartTransaction())
		{
			...
			{
				clickedProject = newProject;
				setDetailFragment(ProjectDetailFragment.ARG_VIEW_DETAIL,clickedProject);
				if(AssociationEndMultiplicity == ToMANY)
				{
					list_fragment.setActivatedPosition(clickedProject);
					list_fragment.setSelection(clickedProject);
				}
			}
		}
		else
	...
	{
		if(caller == Project.class && member == null && worker == null && qualification == null && company == null)
