...
public class CompanyListViewHolder implements ListViewHolder, Serializable{
	private transient ViewHolder holder;
	private final int layout = R.layout.company_view_list;
	private final int contractError = R.id.default_list_invalid;
	private final int Company_name = R.id.company_list_name_value;
	
	private class ViewHolder
	{
		public ImageView errorIcon;
		public TextView Company_nameView;
		
		public ViewHolder(View convertView)
		{
			this.errorIcon = (ImageView) convertView.findViewById(contractError);
			this.Company_nameView = (TextView) convertView.findViewById(Company_name);
		}
	}
	...
	public View setViewHolderContent(final View convertView, final Object object)
	{
		if(object != null)
		{
			...
			if(holder.Company_nameView != null)
				holder.Company_nameView.setText("" + ((Company) object).name());
			if(((Company) object).isAssociationRestrictionsValid())
			...
			holder.errorIcon.setOnClickListener(new OnClickListener()
			{
				...
				{
					Transactions.addSpecialCommand(new Command(CompanyListViewHolder.class, CommandType.READ, CommandTargetLayer.VIEW, 0, null, null, null, 0, null, null));
					String message = "";
					if(((Company) object).employees().size() < 1)
						message += "This Company must have at least 1 employees\n";
