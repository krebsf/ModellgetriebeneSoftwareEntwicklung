...
import org.quasar.IceCream3.businessLayer.ModelMusts;
import org.quasar.IceCream3.utils.UtilNavigate;
import org.quasar.IceCream3.businessLayer.Address;
import org.quasar.IceCream3.businessLayer.CalendarDate;
...
//												Log.i("server - delete", "fiz o delete do " + c.getoldObjectID());
							...
							if(transaction != null)
								UtilNavigate.showWarning(IceCreamMemory.getActiveActivity(), "Connection Warning", "Connection to server was made\nConnection to the database was made\nFailed in setting the transaction: " + transaction.toString() + "\nrolling back changes");
							else
								UtilNavigate.showWarning(IceCreamMemory.getActiveActivity(), "Synchronization", "You have not made any changes\nThere are not any transactions");
							session.rollback();
						} finally {
					...
					error = true;
					UtilNavigate.showWarning(IceCreamMemory.getActiveActivity(), "Connection Warning", "Connection to server failed\ncheck if you are connected to the internet");
				}
				if(!error)
					UtilNavigate.showWarning(IceCreamMemory.getActiveActivity(), "objects sent", "objects sent ");
				return null;
			}
		}.execute();
...
//				Log.i("pedir ver objectos todos", "" + "ja pedi ao server");
				...
				if(client != null){
					try {
						...
						error = true;
						UtilNavigate.showWarning(IceCreamMemory.getActiveActivity(), "Connection Warning", "Connection to server sucess\nproblem getting session");
					}
					...
					if(session != null){
						try {
							...
							allObjects.addAll(session.query(Address.class));
							allObjects.addAll(session.query(CalendarDate.class));
							allObjects.addAll(session.query(Entry.class));
							allObjects.addAll(session.query(Station.class));
							for (Object objectToUpdate : allObjects) {
							...
							error = true;
							UtilNavigate.showWarning(IceCreamMemory.getActiveActivity(), "Connection  problem", "problem during the storage\nrolling back changes");
						} finally {
							session.close();
							client.close();
						}
				...
				if(!error)
					UtilNavigate.showWarning(IceCreamMemory.getActiveActivity(), "Synchronization sucess", "new DB is ready for use");
				return null;
			}
